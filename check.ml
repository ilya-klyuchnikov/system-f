...
let got_exp got exp =
  raise
    (Type_error
       ("got " ^ Syntax.string_of_type got ^ " where " ^ exp ^ " expected"))

let got_hole msg = raise (Type_error ("got type with holes in " ^ msg))

(* Asserts that the given type is int. *)
let assert_int t0 = match view_type t0 with IntT -> () | t -> got_exp t "int"

(* Asserts that the given type is a function type. *)
let assert_arr t0 =
  match view_type t0 with ArrT _ -> () | t -> got_exp t "arrow type"
...
let assert_same_type t1 t2 =
  let rec do_assert = function
    ...
    | AllT (n1, t1), AllT (n2, t2) when n1 = n2 -> do_assert (t1, t2)
    | _ ->
        raise
          (Type_error
...
let un_arr i t0 =
  match view_type t0 with
  | ArrT (ts, tr) as t ->
      if i = List.length ts then (ts, tr)
...
let rec inst_all holes n = function
  | IntT -> IntT
  | ArrT (ts, tr) ->
      ArrT (List.map ~f:(inst_all holes n) ts, (inst_all holes n) tr)
  | TupT ts -> TupT (List.map ~f:(inst_all holes n) ts)
  | VarT m ->
      if m < n then VarT m
      else if m - n < List.length holes then List.nth_exn holes (m - n)
...
let rec shift_type tau depth shift =
  ...
  | TupT ts -> TupT (List.map ~f:(fun t -> shift_type t depth shift) ts)
  | VarT m -> if m >= depth then VarT (m + shift) else VarT m
  | AllT (n, t) -> AllT (n, shift_type t (depth + n) shift)
  | HoleT { contents = Some t } -> shift_type t depth shift
...
let rec type_subst tau1 n tau =
  ...
  | IntT -> IntT
  | ArrT (ts, tr) ->
      ArrT (List.map ~f:(fun t -> type_subst t n tau) ts, type_subst tr n tau)
  | TupT ts -> TupT (List.map ~f:(fun t -> type_subst t n tau) ts)
  | VarT m ->
      (* More Arithmetic TODO *)
      if n = m then shift_type tau 0 m
      else if n < m then VarT (m - 1)
      else VarT m
  | AllT (m, t) -> AllT (m, type_subst t (n + m) tau)
...
let rec shift_expr_types e depth shift =
  match e with
  | VarE x -> VarE x
  | LetE (bindings, body) ->
      LetE
        ( List.map bindings ~f:(fun (x, e, t) ->
              (x, shift_expr_types e depth shift, shift_type t depth shift)),
          shift_expr_types body depth shift )
  | IntE n -> IntE n
  | SubE (e1, e2) ->
      SubE (shift_expr_types e1 depth shift, shift_expr_types e2 depth shift)
  | If0E (e1, e2, e3) ->
      If0E
        ( shift_expr_types e1 depth shift,
          shift_expr_types e2 depth shift,
          shift_expr_types e3 depth shift )
  | TupE es -> TupE (List.map ~f:(fun e -> shift_expr_types e depth shift) es)
  | PrjE (e, ix) -> PrjE (shift_expr_types e depth shift, ix)
  | LamE (args, body) ->
      LamE
        ( List.map ~f:(fun (x, t) -> (x, shift_type t depth shift)) args,
          shift_expr_types body depth shift )
  | AppE (e0, es) ->
      AppE
        ( shift_expr_types e0 depth shift,
          List.map ~f:(fun e -> shift_expr_types e depth shift) es )
  | FixE (x, t, e) ->
      FixE (x, shift_type t depth shift, shift_expr_types e depth shift)
  | LAME (n, e) -> LAME (n, shift_expr_types e (depth + n) shift)
  | APPE (e, ts) ->
      APPE
        ( shift_expr_types e depth shift,
          List.map ~f:(fun t -> shift_type t depth shift) ts )
  | HoleE { contents = e } -> HoleE (ref (shift_expr_types e depth shift))
...
let rec kc typevars_env = function
  ...
  | VarT n ->
      raise
        (Type_error ("unbound type variable: de Bruijn index " ^ Int.to_string n))
...
and tc_infer ((typevars_env, termvars_env) as env) = function
  ...
  | If0E (e1, e2, e3) ->
      ...
      t2
  | TupE es -> TupT (List.map ~f:(tc_infer env) es)
  | PrjE (e, ix) -> prj_tup (tc_infer env e) ix
  | LamE (xts, body) ->
  ...
  | FixE (x, t, e) ->
      ...
      t
  | LAME (n, e) ->
      let termvars_env' = Env.map (fun _ t -> shift_type t 0 n) termvars_env in
...
and tc_check ((typevars_env, termvars_env) as env) exp typ =
  ...
  | LamE (xts, body), typ ->
      ...
      List.iter2_exn xts ts ~f:(fun (x, t) t' ->
  ...
  | LAME (n, e), typ ->
      ...
      if n = m then
        let termvars_env' =
          Env.map (fun _ t -> shift_type t 0 n) termvars_env
        in
        tc_check (n + typevars_env, termvars_env') e t
      else got_exp typ ("Lam that takes " ^ string_of_int m ^ " type variables")
  | HoleE e, typ -> tc_check env !e typ
  | exp, typ ->
