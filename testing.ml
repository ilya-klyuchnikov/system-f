...
(*
    make_check_equal : ?string -> ?('a -> string) -> unit -> ?string ->
                       ...
                       unit
 *)
let make_check_equal ?(test_module = "") ?(to_string = fun _ -> "#<abstr>") ()
    ?(name = "") compute_actual expected =
  let actual =
    try compute_actual ()
    with any_exn ->
      raise
  ...
  if actual = expected then ()
    ...
    raise
      (Check_equal_fail (test_module, name, to_string actual, to_string expected))
