...
let rec sexp_of_sexp type_env = function
  ...
  | TupT ts -> S.List (S.Atom "*" :: List.map ~f:(sexp_of_sexp type_env) ts)
  | VarT n -> (
  ...
  | HoleT { contents = Some t } -> sexp_of_sexp type_env t
  | HoleT { contents = None } -> S.Atom "_"
...
let rec normalize_complete_type = function
  | IntT -> IntT
  | ArrT (ts, tr) ->
      ArrT (List.map ~f:normalize_complete_type ts, normalize_complete_type tr)
...
let rec fv e0 =
  ...
  let remove_bindings remv bindings fvset =
    List.fold ~f:Set.remove ~init:fvset (List.map ~f:remv bindings)
  in
  match e0 with
  | VarE x -> Set.singleton x
  | LetE (bindings, body) ->
      remove_bindings (fun (x, _, _) -> x) bindings (fv body)
  | IntE _ -> Set.empty
...
let rec normalize_expr = function
  ...
  | LetE (xes, body) ->
      LetE
        ( List.map ~f:(fun (x, e, t) -> (x, normalize_expr e, t)) xes,
          normalize_expr body )
  | IntE n -> IntE n
  | SubE (e1, e2) -> SubE (normalize_expr e1, normalize_expr e2)
  | If0E (e1, e2, e3) ->
      If0E (normalize_expr e1, normalize_expr e2, normalize_expr e3)
  | TupE es -> TupE (List.map ~f:normalize_expr es)
  | PrjE (e, ix) -> PrjE (normalize_expr e, ix)
  | LamE (xts, body) -> LamE (xts, normalize_expr body)
  | AppE (e0, es) -> AppE (normalize_expr e0, List.map ~f:normalize_expr es)
  | FixE (x, t, e) -> FixE (x, t, normalize_expr e)
  | LAME (n, e) -> LAME (n, normalize_expr e)
  | APPE (e, ts) -> APPE (normalize_expr e, ts)
  | HoleE { contents = e } -> normalize_expr e
