...
let check_equal_t =
  Testing.make_check_equal ~test_module:"Parser" ~to_string:string_of_type ()
...
let assert_not_keyword x = if is_keyword x then stx_err "identifier" (S.Atom x)
...
let rec type_of_sexp type_env = function
  ...
  | S.List [ S.Atom "all"; S.List new_type_vars; arg ] ->
      ...
      AllT (List.length btv, type_of_sexp (List.rev btv @ type_env) arg)
...
let () =
  check_equal_t
    ~name:"type_of_sexp: (int int -> (* int int) (int -> int) -> int -> int)"
    (fun () ->
      type_of_string "(int int -> (* int int) (int -> int) -> int -> int)")
    (ArrT
       ( [ IntT; IntT ],
         ArrT
           ( [ TupT [ IntT; IntT ]; ArrT ([ IntT ], IntT) ],
             ArrT ([ IntT ], IntT) ) ))
...
let bindings_of_let_sexps xt_of_sexp =
  List.map ~f:(bindings_of_let_sexp xt_of_sexp)
...
let bindings_of_lam_vars t_of_sexp =
  List.map ~f:(bindings_of_lam_sexp t_of_sexp)
...
let rec expr_of_sexp type_env sexp0 =
  ...
  | S.List ss -> (
      ...
      | [ S.Atom "if0"; e1; e2; e3 ] ->
          If0E
            ( expr_of_sexp type_env e1,
              expr_of_sexp type_env e2,
              expr_of_sexp type_env e3 )
      | S.Atom "tup" :: es -> TupE (List.map ~f:(expr_of_sexp type_env) es)
      | [ S.Atom "prj"; e; S.Atom ix ] ->
          let ix =
            try int_of_string ix
      ...
      | [ S.Atom "lam"; S.List bindings; body ] ->
          HoleE
            (ref
               (LamE
                  ( bindings_of_lam_vars (type_of_sexp type_env) bindings,
                    expr_of_sexp type_env body )))
      ...
      | [ S.Atom "Lam"; S.List new_type_vars; body ] ->
          let btv = List.map ~f:type_var_of_sexp new_type_vars in
          LAME (List.length btv, expr_of_sexp (List.rev btv @ type_env) body)
      | S.Atom "@" :: e :: ts ->
          APPE (expr_of_sexp type_env e, List.map ~f:(type_of_sexp type_env) ts)
      | S.Atom op :: _ when is_keyword op -> stx_err op sexp0
      | e0 :: es ->
          AppE
...
let () =
  check_equal_any () ~name:"expr_of_string  (Lam (a). (lambda (x:a). x))"
    (fun () ->
      let e = expr_of_string "(Lam (a) (lam ((x a)) x))" in
      normalize_expr e)
...
let () =
  check_equal_any () ~name:"expr_of_string  (lambda (x:int  y:_). x)"
    (fun () ->
      let e = expr_of_string "(lam ((x int)  y) x)" in
      normalize_expr e)
...
let () =
  check_equal_any ()
    ~name:
      "expr_of_string  (Lam (a b). (let ((fst (lambda ((x a) (y b)) x)) fst)))"
    (fun () ->
      let e =
        expr_of_string "(Lam (a b) (let ((fst (lam ((x a) (y b)) x))) fst))"
      in
      normalize_expr e)
    (LAME
       ( 2,
...
let () =
  check_equal_any ()
    ~name:"expr_of_string  (let ((f (lam (x) x) (-> int int))) 0)"
    (fun () ->
      let e = expr_of_string "(let ((f (lam (x) x) (-> int int))) 0)" in
      normalize_expr e)
    (LetE
       ( [
           ( "f",
...
let () =
  check_equal_any ()
    ...
    (LAME
       ( 2,
         LamE
           ( [ ("k", AllT (1, ArrT ([ ArrT ([ VarT 2 ], VarT 0) ], VarT 0))) ],
             APPE (VarE "k", [ VarT 0 ]) ) ))

let () =
  check_equal_any () ~name:"Lam a. (lam ([x a])  (Lam b. (lam ([y b]) x)))"
    (fun () ->
      let e =
        expr_of_string
          ( "(Lam (a)" ^ "  (lam ((x a))" ^ "    (Lam (b)"
    ...
    (LAME
       ( 1,
         LamE ([ ("x", VarT 0) ], LAME (1, LamE ([ ("y", VarT 0) ], VarE "x")))
       ))
